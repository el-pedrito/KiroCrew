...
import { editQueuedMessage } from '../src/store/chatSlice'
...
describe('ChatPane — queued message inline edit (issue #2240)', () => {
  ...
  it('does not duplicate a committed edit into the composer when only the response is lost', async () => {
    // Transport-ambiguous failure: the server commits and broadcasts, then
    // the HTTP response is lost. The PATCH rejects, but the authoritative
    // queue_edit echo has already applied the edit - the catch must NOT
    // re-seed the composer with already-committed text.
    server.use(
      http.patch('/api/chat/slots/' + SLOT + '/queue/q1', async () => {
    ...
    const input = await screen.findByRole('textbox', { name: 'Edit queued message' })
    await user.clear(input)
    await user.type(input, 'committed but response lost{Enter}')

    // The authoritative WS echo lands before the rejection does.
    store.dispatch(editQueuedMessage({ slot: SLOT, queue_id: 'q1', content: 'committed but response lost' }))

    // The card shows the committed edit...
    await waitFor(() => {
      const msgs = store.getState().chat.slotMessages[SLOT] || []
      const m = msgs.find(x => x.role === 'queued' && (x.meta as any)?.queueId === 'q1')
      expect(m?.content).toBe('committed but response lost')
    })
    // ...and after the rejection fires, the composer is NOT re-seeded.
