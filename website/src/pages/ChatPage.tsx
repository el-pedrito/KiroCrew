...
import {
  ...
  setSlotRunning, startLocalTurn, syncSlotRunningFromServer, setPendingInput, resolveByApprovalId, clearPendingPermissions, cancelQueuedMessage, selectSlotMessages,
  selectComposerBusy,
  selectContinuable,
 ...
 // composer instead of auto-submitting. Auto-submitting would be a
  ...
  const handleEditQueued = useCallback((queueId: string, content: string) => {
    ...
    api.editQueuedMessage(slot, queueId, trimmed).catch(() => {
      // Transport-ambiguous failure guard: when the server committed but the
      // HTTP response was lost, the authoritative queue_edit echo has
      // typically already applied this edit - restoring then would duplicate
      // committed text. Read the store at catch time (the closure predates
      // the echo) and skip the restore when the card already carries this
      // exact edit. Works for background slots too: the echo updates the
      // store regardless of which slot is active.
      const committed = selectSlotMessages(store.getState(), slot)
        .some(m => m.role === 'queued' && (m.meta as any)?.queueId === queueId && m.content === trimmed)
      if (committed) return
